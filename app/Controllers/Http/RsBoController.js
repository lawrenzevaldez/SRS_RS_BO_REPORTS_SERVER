...
class RsBoController {
  ...
  async getBranchDetails({ request, response }) {
    try {
      let { branch_code, p_page, p_search, p_currentSort, p_currentSortDir } =
        request.only([
          "branch_code",
          "p_page",
          "p_search",
      ...
      let [res, page_count] = await RsBoMod.getBranchDetails(
        branch_code,
        p_page,
        p_search,
        p_currentSort,
      ...
      if (!res) {
        throw new CustomException(
          { message: "UNABLE TO FETCH BRANCH RS & BO DETAILS" },
          401
        );
      }
      ...
      console.log("get_branch_details error", e);
      response.status(500).send({ message: "CAN'T CONNECT TO BRANCH" });
    }
  }
  ...
  async getBranchDetailsByDate({ request, response }) {
    ...
    let [res, page_count] = await RsBoMod.getBranchDetailsByDate(
      branch_code,
      p_page,
      p_search,
      p_currentSort,
      p_currentSortDir,
      moment(d_from).format("YYYY-MM-DD"),
      moment(d_to).format("YYYY-MM-DD")
    );
  ...
  async getMovementDetails({ request, response }) {
    try {
      let { branch_code, movement_no, rs_id } = request.only([
        "branch_code",
        "movement_no",
        "rs_id",
      ]);
      ...
      let movement_header = await RsBoMod.getMovementHeader(
        branch_code,
        fMovement_no
      );

      let movement_id = movement_header.MovementID;
      let movement_details = await RsBoMod.getMovementDetails(
        branch_code,
        movement_id
      );

      let getBranchDescription = await RsBoMod.getBranch(branch_code);
      ...
      res.push({
        ...
        branch_name: getBranchDescription.name,
      });
  ...
  async exportMovementDetails({ request, response }) {
    ...
    worksheet
      .getRow(1)
      .getCell(1).value = `Movement No. ${movement_no} Details`;
    ...
    worksheet.getRow(5).getCell(5).value = moment(
      movement_header.PostedDate
    ).format("YYYY-MM-DD");
    ...
    // PICKUP FORM DATA
    worksheet.getRow(4).getCell(8).value =
      pickup_details == null ? "" : pickup_details.tname;
    worksheet.getRow(5).getCell(8).value =
      pickup_details == null ? "" : pickup_details.tplate_no;
    ...
    for (const row of movement_details) {
      ...
      worksheet.getRow(ctr).getCell(7).value = parseFloat(
        (row.unitcost * row.qty).toFixed(2)
      );
      ctr++;
    }
    ...
    workbook.xlsx.writeFile(
      Helpers.publicPath() + "/files/" + `RSID${rs_id}.xlsx`
    );
  ...
  async getRmsDetails({ request, response }) {
    let { branch_code, rs_id, supplier_code } = request.only([
      "branch_code",
      "rs_id",
      "supplier_code",
    ]);
    let res = await RsBoMod.getRmsDetails(branch_code, rs_id, supplier_code);
    response.status(200).send(res);
  }

  async getRsCvNo({ request, response }) {
    let { branch_code, movement_no } = request.only([
      "branch_code",
      "movement_no",
    ]);
    let res = await RsBoMod.getRsCvNo(branch_code, movement_no);
    response.status(200).send(res);
  }

  async getRmsExtended({ request, response }) {
    let { branch_code, rs_id } = request.only(["branch_code", "rs_id"]);
    let res = await RsBoMod.getRmsExtended(branch_code, rs_id);
    let total = 0;
    for (const row of res) {
      total += parseFloat(row.qty * row.price);
    }
